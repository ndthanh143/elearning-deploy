...
export function StudentTab() {
  ...
  const userInstance = userKeys.listStudent({ page, size: PAGE_SIZE })
  ...
  const { mutate } = useMutation({
    mutationFn: userService.updateStatusStudent,
    onSuccess: () => {
      refetch()
  ...
  const handleActive = () => {
    selectedActive && mutate({ studentId: selectedActive, status: 1 })
  }
  const handleDeactivated = () => {
    selectedDeactivated && mutate({ studentId: selectedDeactivated, status: 0 })
  }
